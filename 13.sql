...
--1 селекты с этими функциями(вернуть дату найма и количество дней, месяцев и лет, которые прошли до настоящего времени)
SELECT 
  name,
  hire_date,
...
--2 вернуть дату найма в разных форматах(например 01.01.1987, 01 января 2007,три разные штуки)
...
SELECT 
  ...
  -- Формат 2: 01 января 2007
  TO_CHAR(hire_date, 'DD Month YYYY', 'NLS_DATE_LANGUAGE=RUSSIAN') AS format_2,

  -- Формат 3: 01-01-2007
  TO_CHAR(hire_date, 'DD"-"FMMonth"-"YYYY" года"', 'NLS_DATE_LANGUAGE=RUSSIAN') AS formatted_date

FROM 
  salesreps;

--3 заменить букву А большую на а маленькую в фамилиях и именах сотрудников
...
--4 дополнить список фамилий пробелами до длины 20
...
--5 обрезать в полученном списке пробелы обратно
...
--6 применить функции транк и раунд к датам, пояснить что получилось(?)
...
--7 сформировать емейл адрес из фамилии и имени сотрудников

SELECT
  name AS original_name,

  -- имя (все символы до первого пробела)
  SUBSTR(name, 1, INSTR(name, ' ') - 1) AS first_name,

  -- фамилия (все символы после первого пробела)
  SUBSTR(name, INSTR(name, ' ') + 1) AS last_name,

  -- сформируем email в формате фамилия_имя@gmail.com
  LOWER(
...
--транк движение вниз, а раунд это математика
